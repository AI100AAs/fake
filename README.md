...
SignalCheck is an educational news-literacy workspace for slowing down a story,
separating checkable claims from framing, and deciding what needs independent
verification. It is built for UBC's AI 100 course and keeps the reader in the
loop: a score is a signal, not proof of truth.

## What It Does

- Accepts pasted article text for a fast, local heuristic analysis.
- Accepts a public article URL, extracts readable text, and asks the course AI
  model for a claim-level report.
- Shows a bounded credibility signal, summary, extracted claims, assessments,
  evidence explanations, source links, and signals to investigate.
- Saves completed analyses in SQLite so earlier checks can be restored.
- Provides a persistent knowledge base for trusted sources, class notes, and
  background facts.
- Cross-references extracted claims with saved references using shared terms and
  displays the matching terms and source links beneath each claim.
- Includes an evidence-desk chat that answers questions using the saved
  knowledge base and the currently loaded story, while stating when evidence is
  missing or uncertain.
- Uses a responsive text-first interface with light/dark mode and no public
  starter/admin chrome.

The AI-backed URL analysis and evidence-desk chat run only after a user action.
They use the app's separate `GIZMO_LLM_*` credentials when available and fail
closed with a readable error when the course model is unavailable.

## Technology

- Python `Flask` backend with prefix-safe JSON routes
- SQLite persistence for article history and knowledge-base references
- Build-free HTML, CSS, and JavaScript frontend
- Text shell selected through `deploy/app-shell.txt`
- Optional GizmoApp capability modules retained in the scaffold but disabled by
  default
...
- `POST /api/analyze` fetches a public article URL and returns the AI report plus extracted text
- `GET /api/history` lists saved article analyses
- `POST /api/history` saves an analysis for later restoration
- `GET /api/knowledge-base` lists saved references
- `POST /api/knowledge-base` adds a reference with a title, notes, and optional source URL
- `DELETE /api/knowledge-base/<id>` removes a saved reference
- `POST /api/chat` asks the evidence desk about the saved references and current story
...
The public interface is available at the root by default. If
`GIZMOAPP_URL_PREFIX=/signalcheck` is configured, the same routes are served
under `/signalcheck`, for example `/signalcheck/api/chat`.

`POST /api/analyze` is intentionally limited to public `http://` and `https://`
URLs and rejects private or local network targets. The server extracts at most
30,000 characters of article text. `POST /api/chat` accepts messages up to
1,200 characters and supplies the model only the saved reference shelf and the
...
Generated deployment instructions use the host's HTTPS URL. SignalCheck does
not register a service worker or advertise offline installation; add a versioned
cache and explicit offline/update UX only if the project requirements genuinely
